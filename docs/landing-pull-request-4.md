...
`origin/main` is at `3b5f6cc`. That commit is already the base of the branch. `git merge origin/main`
answered "Already up to date". The merge wrote no commit. The merge changed no file. I did not
rebase. I did not force push.

I pushed the branch. Git answered "Everything up-to-date". GitHub reports the pull request as
...
Every gate that can run here passes. I ran each gate as one command. I read each result before I ran
...
The failing check is `plan and apply`. It belongs to the `deploy` workflow. That workflow starts on
a push to `main`. It also starts on a manual start. It does not start on a pull request. So
`gh pr checks 4` prints two checks and exits 0. It does not print this one. Read the commit instead:
...
The deploy worked. Terraform planned. Terraform applied. The function url answered 200. The front
page answered 200.
...
1. The deployed page. `https://dm60tfc2fh2e6.cloudfront.net/videos?id=gyN9lV9QgyA` answered 502 with
   4777 bytes. Its title is "The platform refused this page, not your video".

2. The function log. Log group `/aws/lambda/transcript-handler` in eu-west-1, at
   2026-09-01T07:55:53.207Z, one second after my own request:

       {"event":"watch_page_unreadable","video_id":"gyN9lV9QgyA","cause":"bot_check",
        "status":200,"bytes":1381407,"has_video_details":true,"has_caption_tracks":false,
...
3. The same code, run from this sandbox. `npm run test:live` exited 0. It read 560 caption segments
...
I read the log of run 33313972826, which is `main` at `3b5f6cc`. It failed on the same step. It
answered 404 eight times. Before this pull request the page called a refusal a missing video. After
this pull request the page answers 502 and names the refusal. The check is still red. It is now red
...
- I cannot test a fix from here. This sandbox is not refused, so a change made here passes for the
  wrong reason. The only place to test it is a real deploy, and a deploy from a sandbox is not
  allowed.
...
address. Somebody must choose one of these. Each one costs money while nobody visits:
