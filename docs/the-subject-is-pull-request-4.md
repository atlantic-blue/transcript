...
Eleven pull requests are open. They are numbers 4 to 14.
...
Pull request 4 opened on 31 August 2026 at 11:30. It is the oldest of the eleven. It is the only
one with a check that is not passing. It qualifies on that check. It does not qualify on a conflict.
...
zero contexts. That is an absence, not a failure. It is the same on all eleven heads.
...
The trigger is the cause. `plan and apply` is the only job in the `deploy` workflow. That workflow
starts on a push to `main` and on a manual start. Its `on:` block holds `push` and
`workflow_dispatch`. It holds no `pull_request`. So GitHub keeps the run out of the pull request
summary. The run on this head commit started as `workflow_dispatch`.

An absent check reads the same as a passing one. Read the check runs on the head commit, never the
...
## The other ten open pull requests

Each one is newer than pull request 4. Each one has two check runs, `code` and `infrastructure`, and
both pass. None conflicts with `main`. So none qualifies.
...
Pull requests 5, 6, 7, 9, 11 and 13 are the output of earlier rounds of this same task. Each one
names pull request 4. Pull requests 8, 10, 12 and 14 report the round that worked it.

## The exclusions I checked

- **Draft.** No pull request is a draft. `isDraft` is false on all eleven.
...
- **Conflicts.** GitHub answers `MERGEABLE` and `CLEAN` for all eleven. I did not trust that alone.
  I ran `git merge-tree --write-tree origin/main <head>` for each one. Each exits 0 and prints no
  conflict line.
...
Then the other two conflict on add over add, and a person must resolve them by hand.

The pull request that carries this report adds `docs/the-subject-is-pull-request-4.md`. No other
open pull request touches that path, so it conflicts with none of the ten.

## What landing pull request 4 will find

An earlier round worked pull request 4 to the end. Its report is at
`/home/agent/shared/land/land.md`, and it also ships as pull request 10 and pull request 14. The
short version:
...
- Every `deploy` run in this repository failed, on `main` as well as on this branch. So pull
  request 4 does not cause the failure. It inherits it.
- The cause is the address the function calls from. The platform sends a bot check to Amazon Web
  Services Lambda in eu-west-1. The same code, run from this sandbox, reads 280 caption segments.

I did not verify those readings again in this round. I verified the state that decides the pick: the
pull request list, the draft flag, the base branch, the mergeability, and the check runs on every
head commit.
